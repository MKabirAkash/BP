...
function StudentBanModal({ type, banModalClose }) {
  return (
    <div className="bg-white shadow-5 w-full md:w-[700px] rounded-xl">
      <p className="w-full border-b border-web_clr border-opacity-25 py-3 flex justify-between px-6">
        <span className="pl-3 inline-block">
          {type && type === "ban"
            ? "Why are you banning this student ?"
            : "Send message"}
        </span>{" "}
        ...
        <div className="w-full">
          <div className="w-full flex flex-col">
            ...
            <textarea
              ...
              cols="50"
              rows="10"
              className="border border-web_clr border-opacity-25 rounded-xl w-full mt-2"
            ></textarea>
          </div>
        </div>
      </section>
...
export default StudentBanModal;
